...
import { getResidentPreferences, updateResidentPreferences, type Preferences } from "@/lib/api"
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function loadPreferences() {
      ...
      try {
        setLoading(true)
        const response = await getResidentPreferences(userEmail)
        if (response.success) {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true)
      const response = await updateResidentPreferences(userEmail, preferences)
      if (response.success) {
        toast({
          title: "Succès",
