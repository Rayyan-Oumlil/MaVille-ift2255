...
import { getStpmProblemes, getStpmCandidatures, getResidentsTravaux } from "@/lib/api"
...
export default function ActivityChart() {
  ...
  const [chartData, setChartData] = useState<{ date: string; problems: number; projects: number; applications: number }[]>([])
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        const [problemsRes, applicationsRes, projectsRes] = await Promise.all([
          getStpmProblemes({ page: 0, size: 1000 }).catch((err) => {
            console.warn("Erreur lors de la récupération des problèmes pour le graphique:", err)
            return { data: [], totalItems: 0, total: 0, pageSize: 0, size: 0 }
          }),
          getStpmCandidatures({ page: 0, size: 1000 }).catch((err) => {
            console.warn("Erreur lors de la récupération des candidatures pour le graphique:", err)
            return { data: [], totalItems: 0, total: 0, pageSize: 0, size: 0 }
          }),
          getResidentsTravaux({ page: 0, size: 1000 }).catch((err) => {
            console.warn("Erreur lors de la récupération des travaux pour le graphique:", err)
            return { data: [], totalItems: 0, total: 0, pageSize: 0, size: 0 }
          }),
        ])
