...
import { getStpmProblemes, type Probleme } from "@/lib/api"
...
export default function RecentProblems() {
  const [allProblems, setAllProblems] = useState<Probleme[]>([])
  ...
  useEffect(() => {
    async function fetchProblems() {
      try {
        setLoading(true)
        const response = await getStpmProblemes({ page: 0, size: 100 })
        // Gérer les deux formats possibles (pageSize/totalItems ou size/total)
        ...
        setNewCount(newProblems.length)
      } catch (error) {
        console.error("Error fetching problems:", error)
        // Afficher un message d'erreur plus clair
        if (error instanceof Error) {
          console.error("Error details:", error.message)
          // Si c'est une erreur de connexion, ne pas spammer les logs
          if (error.message.includes('Impossible de se connecter')) {
            console.warn("Backend non accessible. Vérifiez que le serveur est démarré sur http://localhost:7000")
          }
        }
        // Ne pas afficher d'erreur à l'utilisateur, juste laisser la liste vide
        setAllProblems([])
        setNewCount(0)
      } finally {
        setLoading(false)
    ...
    fetchProblems()
    // Refresh every 60 seconds (seulement si pas d'erreur de connexion)
    const interval = setInterval(() => {
      fetchProblems().catch(() => {
        // Ignorer les erreurs silencieusement lors du refresh automatique
      })
    }, 60000)
